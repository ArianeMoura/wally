...
const meses = Array.from(
    { length: 12 },
    (_, i) =>
        format(new Date(2024, i, 1), "MMMM", { locale: ptBR }).replace(
            /^\w/,
            (c) => c.toUpperCase(),
        ),
);
...
export function useWalletViewModel() {
    ...
    const [activeDateTab, setActiveDateTab] = useState<"month" | "year">(
        "month",
    );
    ...
    const {
        ...
        refetch: refetchStatusUsuario,
    } = useQuery<StatusUsuario>({
        ...
        queryFn: async () => {
            ...
            console.log({
                url: `${API_URL}/status/balanco?${searchParams.toString()}`,
            });
            const response = await fetch(
                `${API_URL}/status/balanco?${searchParams.toString()}`,
                {
                    method: "GET",
                    headers: {
        ...
        if (statusUsuario) {
            if (searchQuery.trim() === "") {
                ...
                return statusUsuario.transacoes.filter((transaction) => {
                    ...
                    const descriptionMatch = transaction.nome.toLowerCase()
                        .includes(searchLower);
                    const valueMatch = transaction.valor.toString().includes(
                        searchLower,
                    );
                    const typeMatch = transaction.tipo.toLowerCase().includes(
                        searchLower,
                    );
